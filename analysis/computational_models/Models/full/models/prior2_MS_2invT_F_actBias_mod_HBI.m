function [loglik] = prior2_MS_2invT_F_actBias_mod_HBI(parameters, subj)

%Bayesian learner model with 2 priors and bias in the factory task
%
%parameters:
%mu = mu of the beta distribution that make up each prior (mu(1) = robber, mu(2) = factory)  
%sigma_rel = relative sigma of the beta distribution of each prior 
%(sigma_rel(1) = robber, sigma_rel(2) = factory)  
...
%parameters
mu = 1./(1+exp(-parameters(1:2)));
sigma_rel = 1./(1+exp(-parameters(3:4)));
z =  0.01 * exp(parameters(5:6));
bias = 2 * sigmoid(parameters(7))-1;
...
for i=1:N_stims_tot
    a=mu(task(k))*((1/sigma_rel(task(k)))-1);
    b=(1-mu(task(k)))*((1/sigma_rel(task(k)))-1);
    for t=1:stimTrialList(i)
        ...
        if task(k)==1
            ps1= ps(1);
        elseif task(k)==2
            ps1= softrelu(bias) + (1-softabs(bias))*ps(1); %add actionbias only to factory task
        end
        ps=[ps1 1-ps1];
